...
| `SMARTQ_AGENT_ID` | yes | Agent/profile ID used by Agent mode and reused as `agent_id` in knowledge mode. |
| `SMARTQ_MODEL_ID` | Knowledge mode | Summary model sent as `summary_model_id` to `POST /knowledge-chat/:session_id`. |
