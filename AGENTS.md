...
- `src/lib/` also contains LLM/prompt code, shared Pydantic and TypedDict models, utilities, Wikipedia retrieval, embeddings/reranking, `vectordb.py` hybrid retrieval, and the SmartQ Agent QA SSE client.
- `src/evals/` implements the local SmartQ Agent evaluation CLI: validation-set collection, append-only result records, deterministic retrieval/generation scoring, and JSON/Markdown reports.
...
Mock API and model responses; tests must not require a live SmartQ server, GPU, external Wikipedia connection, or an actual vLLM/embedding-model initialization. SmartQ Agent evaluation tests must mock session creation and SSE events, then assert source-order collection, retrieval-index handling, coverage counts, and saved-run re-scoring without a network call. Persistence tests must use `KnowledgeBase(test=True)` and must not write artifacts under the repository `data/` directory.
